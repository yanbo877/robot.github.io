...
public class DateTimeUtils {
    ...
    public static String getWeeTime() {
        String currentDate = DateTimeUtils.getCurrentDate("yyyy-MM-dd");
        Date date = null;
        try {
            date = DateTimeUtils.strToDate(currentDate, "yyyy-MM-dd");
        } catch (ParseException ex) {
            ex.printStackTrace();
        }
        return DateTimeUtils.getSpecifiedDate(date);
    }
    ...
    public static String formatDateToStr(Date date) {
        ...
        return new SimpleDateFormat(DEFAULT_FORMAT_DATE_TIME).format(date);
    }
    ...
    /**
     * 解析日期字符串为日期：yyyy-MM-dd HH:mm:ss.
     *
     * @param date 日期字符串
     * @return 日期
     */
    public static Date parseStrToDate(String date) {
        return getParseDate(date, DEFAULT_FORMAT_DATE_TIME);
    }
    ...
    public static Date parseStrToDate(String date, String pattern) {
        return getParseDate(date, pattern);
    }

    private static Date getParseDate(String date, String pattern) {
        if (date == null) {
            return null;
        }
        try {
    ...
    public static int differentDaysByMillisecond(Date min, Date max) {
    ...
    public static void main(String[] args) {
        try {
            Date date1 = strToDate("2021-08-11", "yyyy-MM-dd");
            Date date2 = strToDate("2021-08-12", "yyyy-MM-dd");
            int d=differentDaysByMillisecond(date1,date2);
